...
describe('api smoke tests with authentication', function () {
  ...
  it('calls /hello', async function () {
    try {
      ...
      assert.fail('GET hello caught error ' + error);
    }
  });

  // it('calls POST /hello', async function () {
  //   try {
  //     const endpoint = await getEndpoint();
  //     const url = endpoint + '/hello?accessToken=' + user.accessToken;
  ...
  //   } catch (error) {
  //     assert.fail('POST hello caught error ' + JSON.stringify(error));
  //   }
  // });
